...
cmake -S "$SRC" -B "$BUILD_DIR/build$NATIVE_SUFFIX" -G "Unix Makefiles" \
  "${NATIVE_CMAKE_FLAGS[@]}" \
...
cmake --build "$BUILD_DIR/build$NATIVE_SUFFIX" -j6
echo "Done: $BUILD_DIR/build$NATIVE_SUFFIX/libfreetype.a"
